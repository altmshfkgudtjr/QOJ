import { ApiGetLecture, ApiCreateLecture, ApiUpdateLecture } from '../../controller/creation.js'
...
const ManageLecture = ()=> {
	let view = `
		<div class="creation_title noselect">
			...
			<div id="creation_cancel" class="creation_btn pointer">cancel&nbsp; <i class="fas fa-undo"></i></div>
		</div>
...
const ManageLectureEvent = (lecture_id = '')=> {
	if (lecture_id != '') {						// 분반 수정
		ApiGetLecture(lecture_id, (data)=> {
			...
			document.querySelector("#main_manager").value = data['main_id'];
			document.querySelector("#creation_sub").innerHTML = "";
			for (let submanager of data['submanager']) {
	...
	// 취소
	document.querySelector("#creation_cancel").addEventListener("click", CancelCreation);
}
...
const CancelCreation = () => {
	if (!confirm("WARNING! What you wrote is not saved! Don't change your determine?")) return;
	router._goTo("/board");
}
...
const CreateCreation = (lecture_id='')=> {
	...
	if (lecture_id == '') {	// 생성
		ApiCreateLecture(title, main_manager, sub_managers, (data)=> {
			router._goTo("/board");
			Snackbara("Lecture created successful!");
		});
	} else {			// 수정
		ApiUpdateLecture(lecture_id, title, main_manager, sub_managers, (data)=> {
			router._goTo("/board");
			Snackbara("Lecture updated successful!");
		});
	}
}
